...
class Dashboard_Model extends Model
{
	...
	public function getorders($status) {
		...
		$sql = "SELECT * FROM orders LEFT JOIN cart ON cart_id = cart.id ";

		return $this->_get("* orders.*, cookie_id from orders LEFT JOIN cart ON cart_id = cart.id", 'order_status', [$status])[1];
	}

	public function getOrder(string $orderId) {
		return $this->_get("* products.*, cookie_id ,item_count, item_id, product_count, order_status, phone, email, orders.created_at as order_created_at, more_data
		from products join cart ON item_id = products.id join orders ON cart.id=cart_id", 'cookie_id', [$orderId])[1];
	}
